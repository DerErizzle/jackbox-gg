            ...
            const p = c.I.extend({
                ...
                updatePlayer() {
                    ...
                    if (this.grid && "Gameplay" !== n && "Grid" !== i && (this.grid.reset(), this.grid = null), "RoomFull" !== i)
                        if ("GameLocked" !== i) {
                            if ("Lobby" === n) {
                                if (this.hideLobbyButtons(), !e.isAllowedToStartGame) return s()("#lobby-text").html("Lehne dich zurück und entspanne!"), this.showScreen("#state-lobby"), void this.setupWithDollInfo(e.dollInfo);
                                const o = t.lobbyState;
                                "WaitingForMore" === o ? s()("#lobby-text").html("Warte, bis alle Spieler beigetreten sind") : "CanStart" === o ? (s()("#lobby-text").html("Drücke diesen Knopf, wenn alle Spieler beigetreten sind"), s()("#button-startgame").show()) : "Countdown" === o ? (s()("#lobby-text").html("Drücke diesen Knopf, um den Spielstart abzubrechen"), s()("#button-stopcountdown").show()) : "PostGame" === o && (s()("#lobby-text").html("Was möchtet ihr jetzt machen?"), s()(".button-endbuttons").show()), this.showScreen("#state-lobby")
                            } else if ("Gameplay" === n)
                                if ("MakeSingleChoice" === i) {
                                    ...
                                    if (a) {
                                        ...
                                        let h = "";
                                        for (let g = 0; g < l.length; g++) h += `<button data-choice="${g}" class="${l[g].disabled?`background-finger background-finger-${g}`:""} light-text button-choice button-game button-large btn" ${l[g].disabled?"disabled":""}>${l[g].disabled?"&zwnj;":(i=="Gib das Muster durch das Tippen der Knöpfe wieder."?n[e].text.replace("Orange","Orange").replace("Blue","Blau").replace("Green","Grün").replace("Gun","Pistole").replace("Knife","Messer").replace("Hammer","Hammer").replace("Grenade","Granate"):l[g].text)}</button>`;
                                ...
                                const p = null === e.chosen || void 0 === e.chosen,
                                    ...
                                    b = e.choices;
                                if (p) {
                                    s()("#make-many-choices-text").html(`<p id='many-text'>${m}</p>`), s()("#make-many-choices-sub-text").html("<p id='many-sub-text'>Tippe auf alle zutreffenden Antworten.</p>");
                                    let w = "";
